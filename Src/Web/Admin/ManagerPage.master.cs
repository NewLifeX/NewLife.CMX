...
public partial class ManagerPage : System.Web.UI.MasterPage
{
    ...
    public static void SetFormScript(Boolean IsForm)
    {
        ...
        if (IsForm)
        {
            ...
            p.ClientScript.RegisterClientScriptBlock(p.GetType(), "alert", @"$(function(){$('.form1').validate({errorPlacement: function (lable, element) {element.ligerTip({ content: lable.html(), appendIdTo: lable });},success: function (lable) {lable.ligerHideTip();}});});", true);
        }
    }
}
