    ...
    public class UpdateImageLoad : IHttpHandler
    {
        ...
        public String DefaultImagePath
        {
            get
            {
                ...
                {
                    _DefaultImagePath = NewLife.Configuration.Config.GetConfig<String>("DefaultImagePath");
                }
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception((Int32)AjaxStatus.NoConfig + "-无法获取到配置信息！");
                }
                else
                {
                ...
                {
                    throw new Exception((Int32)AjaxStatus.NoFile + "-请选择上传文件！");
                }
                ...
                if (FailFile.Count > 0)
                {
                    throw new Exception((Int32)AjaxStatus.Error + "-上传失败的文件列表，" + String.Join(",", FailFile.ToArray()));
                }
                //context.Response.Write(AjaxStatusEnum.Success);
                ResultDic.Add("StatusCode", "200");
                ResultDic.Add("Result", (AjaxStatus.Success).ToString());
